...
@php
    ...
    $pendingRequests = $requests->filter(fn($item) => strtolower($item->status ?? '') === 'pending')->count();
    $completedRequests = $requests->filter(fn($item) => strtolower($item->status ?? '') === 'completed')->count();
    $cancelledRequests = $requests->filter(fn($item) => strtolower($item->status ?? '') === 'cancelled')->count();
@endphp
...
<style>
    ...
    .requests-page .stats-icon.danger { background: linear-gradient(135deg, #dc2626, #ef4444); }
    ...
    .requests-page .col-type { width: 140px; }
    ...
    .requests-page .badge-status-pending {
        background: #fff7ed;
        color: #c2410c;
    }
                ...
                <table class="table modern-table">
                    ...
                    <tbody>
                        @forelse($requests as $requestItem)
                            @php
                                $statusClass = match(strtolower($requestItem->status ?? '')) {
