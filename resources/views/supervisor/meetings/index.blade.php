...
@section('content')
@php
    $totalMeetings = $meetings->total() ?? $meetings->count();

    $scheduledCount = collect($meetings->items() ?? $meetings)->filter(function ($meeting) {
        return strtolower($meeting->status ?? '') === 'scheduled';
    })->count();

    $completedCount = collect($meetings->items() ?? $meetings)->filter(function ($meeting) {
        return strtolower($meeting->status ?? '') === 'completed';
    })->count();

    $cancelledCount = collect($meetings->items() ?? $meetings)->filter(function ($meeting) {
        return strtolower($meeting->status ?? '') === 'cancelled';
    })->count();
@endphp
...
<style>
    ...
    .meetings-page .stats-icon.danger { background: linear-gradient(135deg, #dc2626, #ef4444); }
    ...
    .meetings-page .col-project { width: 190px; }
    ...
    .meetings-page .badge-status-scheduled {
        background: #fff7ed;
        color: #c2410c;
    }
    ...
    .meetings-page .badge-status-cancelled {
        background: #fef2f2;
        color: #dc2626;
    }
                    ...
                    <tbody>
                        @forelse($meetings as $meeting)
                            @php
                                ...
                                $statusClass = match(strtolower($meeting->status ?? 'scheduled')) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge-soft {{ $statusClass }}">
                                        {{ ucfirst(str_replace('_', ' ', $meeting->status ?? 'scheduled')) }}
                                    </span>
                                </td>
